...
public class FormulaWeakening implements Visitor<Formula>{
	...
	public FormulaWeakening(List<String> literals, int weakening_rate, int num_of_weakening_to_apply, Random random) {
	...
	public Formula visit(BooleanConstant booleanConstant) {
		...
		if (numOfAllowedWeakenings > 0) { 	
	    	boolean mutate = (random.nextInt(weakening_rate) == 0);
	    	if (mutate) {
	...
	public Formula visit(Literal literal) {
		...
		if (numOfAllowedWeakenings > 0) {
	    	boolean mutate = (this.random.nextInt(weakening_rate) == 0);
	    	if (mutate) {
	    		numOfAllowedWeakenings--;
	    		// 0: TRUE 1: add disjunct 2:F
	    		int option = this.random.nextInt(3);
	    		if (option == 0) 
	    			current = BooleanConstant.TRUE;
	    		else if (option == 1) {
	    			// weak(a) = a | b
					int new_variable = this.random.nextInt(variables.size());
					while (new_variable == literal.getAtom())
						new_variable = this.random.nextInt(variables.size());
	...
	public Formula visit(FOperator fOperator) {
		...
		if (numOfAllowedWeakenings > 0) {
	    	...
	    	if (mutate) {
    			...
    			int option = this.random.nextInt(6);
	    		...
	    		if (option == 1 && operand instanceof Conjunction && numOfTO < 2) {
	    			...
	    			// weak F a = a1 W a
	    			//Formula new_literal = createVariable(variables.get(this.random.nextInt(variables.size())));
	    			//if (this.random.nextBoolean())
	    				//new_literal = new_literal.not();
	...
	public Formula visit(GOperator gOperator) {
		...
		if (numOfAllowedWeakenings > 0) {
	    	...
	    	if (mutate) {
    	    	...
    	    	int option = this.random.nextInt(7);
    			if (option == 0) 
    				current = BooleanConstant.TRUE;
    			else if (option == 1) {
	    			// weak (G(a)) = a
	...
	public Formula visit(Conjunction conjunction) {
		...
		if (numOfAllowedWeakenings > 0) { 	
	    	...
	    	if (mutate) {
	    		...
	    		numOfAllowedWeakenings--;
	    		int option = this.random.nextInt(4);
	    		if (option == 0)
	    			current = BooleanConstant.TRUE;
	    		else if (option == 1){// weak(a & b) = a
	    			if (current.children().size() > 0) {
		    			int to_be_removed = this.random.nextInt(current.children().size());
	...
	public Formula visit(Disjunction disjunction) {
		...
		if (numOfAllowedWeakenings > 0) {
	    	...
	    	if (mutate) {
	    		// 0: TRUE 1: add disjunct 2:F
	    		numOfAllowedWeakenings--;
	    		int option = this.random.nextInt(2);
	    		if (option == 0) 
	    			current = BooleanConstant.TRUE;
	    		else if (option ==1){
	    			//Formula new_literal = createVariable(variables.get(this.random.nextInt(variables.size())));
...
//		if (numOfInputs > 0)
//			max = numOfInputs;
		int new_variable = this.random.nextInt(max);
		Literal new_literal = createVariable(variables.get(new_variable));
...
//		}
		...
		while ((props.contains(new_literal) || props.contains(new_literal.not())) && trying < 5) {
			trying++;
			new_variable = this.random.nextInt(max);
