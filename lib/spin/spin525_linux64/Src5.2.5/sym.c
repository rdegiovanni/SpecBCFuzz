...
extern int	lineno, depth, verbose, NamesNotAdded, deadvar, has_hidden, m_loss;
extern short	has_xu;
...
{	Lextok *m, *n;
	...
	if (m_loss && t == XS)
	{	printf("spin: line %d %s warning: xs tag not compatible with -m (message loss)",
			p->ln,
			(p->fn != NULL) ? p->fn->name : "stdin",
			(char *)0);
	}
