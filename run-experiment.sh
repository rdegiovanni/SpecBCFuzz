...
do
    ...
    echo "ID: $ID_JOB"
    ./SpecBCFuzz.sh -log=$LOG_DIR-$ID_JOB -lib=$LIB_DIR -id=$ID_JOB -run=$N_RUN -sattimeout=$SAT_TIMEOUT -experiment=$EXPERIMENT -popSize=100 -maxNumOfInd=1000 -specs=${SPECS[$K]} 1> $SLURM_DIR/$ID_JOB-ltl-fuzzer.out 2> $SLURM_DIR/$ID_JOB-ltl-fuzzer.err
