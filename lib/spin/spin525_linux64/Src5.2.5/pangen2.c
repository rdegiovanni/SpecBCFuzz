...
/* NEW 4.2.6 */
	if (Pid == claimnr)
	{
		...
		fprintf(tm, "			(int) depth, frm_st0[t->forw], src_claim[%d]);\n\t\t", e->seqno);
...
{	Symbol *s = n->sym;
	...
	{	if (no_arrays)
		...
		{	if (terse
			...
			{
#if 0
	/* trying to catch arrays that are indexed with an array element in the same array
	...
	{	if (n->lft	/* effectively a scalar, but with an index */
		&& (n->lft->ntyp != CONST
		||  n->lft->val != 0))
		{	fatal("ref to scalar '%s' using array index", (char *) s->name);
	}	}
