...
#======================================================================#
# Tcl/Tk Spin Controller, written by Gerard J. Holzmann, 1995-2010.    #
# See the README.html file for full installation notes.                #
#        http://spinroot.com/spin/whatispin.html                       #
#======================================================================#
set xversion "5.2.5 -- 17 April 2010"
...
# NB on newer cygwin installations, use gcc-3 instead of plain gcc
# replace this in the definitions of CC CC0 and CPP

# set CC   "cc -w -Wl -woff,84"	;# ANSI-C compiler, suppress warnings
# set CC   "cl -w -nologo"	;# Visual Studio C/C++ compiler, prefered on PCs
# set CC   "gcc-3 -w"		;# newer cygwin versions
  set CC   "gcc -w"		;# standard gcc compiler - no warnings
# set CC0  "gcc-3"		;# newer cygwin versions
  set CC0  "gcc"		;# just for testing it it is executable
...
add_log "TclTk Version [info tclversion]/$tk_version\n"
	...
	# try to cope with a known problem with gcc
	if {$Unix == 0} {
		# prefer the latest version of gcc
		if {[auto_execok gcc-4] != ""} {
			set CC "gcc-4 -w"
			set CC0 "gcc-4"
		} else {
			if {[auto_execok gcc-3] != ""} {
				set CC "gcc-3 -w"
				set CC0 "gcc-3"
			} else {
				if {[auto_execok gcc] != ""} {
