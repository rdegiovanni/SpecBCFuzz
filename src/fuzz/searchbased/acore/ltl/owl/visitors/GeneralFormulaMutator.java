package fuzz.searchbased.acore.ltl.owl.visitors;
...
//	}
	...
	public Formula visit(BooleanConstant booleanConstant) {
		...
		if (numOfAllowedMutations > 0) { 	
	    	boolean mutate = (ExperimentSetting.RANDOM_GENERATOR.nextInt(mutation_rate) == 0);
	    	if (mutate) {
	    		this.numOfAllowedMutations --;
	    		int random = ExperimentSetting.RANDOM_GENERATOR.nextInt(2); 
	...
	public Formula visit(Literal literal) {
		...
		if (numOfAllowedMutations > 0) {
	    	...
	    	if (mutate) {
	    		this.numOfAllowedMutations --;
	    		int random = ExperimentSetting.RANDOM_GENERATOR.nextInt(4);
	    		...
	    		if (random == 0)
	    			...
	    			//0:not 1:X 2:F 3:G
					int op = ExperimentSetting.RANDOM_GENERATOR.nextInt(4);
					if (op == 0)
						current = literal.not();
					else if (op == 1)
	...
	public Formula visit(XOperator xOperator) {
		...
		if (numOfAllowedMutations > 0) {
	    	...
	    	if (mutate) {
	    		this.numOfAllowedMutations --;
	    		int random = ExperimentSetting.RANDOM_GENERATOR.nextInt(3);
	    		...
	    		if (random == 0)
	    			...
	    			//0:not 1:F 2:G
    				int op = ExperimentSetting.RANDOM_GENERATOR.nextInt(3);
    				if (op == 0)
    					current = operand.not();
					...
					//0:not 1:F 2:G
					int op = ExperimentSetting.RANDOM_GENERATOR.nextInt(7);
					if (op == 0)
						current = current.not();
					else if (op == 1 && numOfTO < 2)
						...
						if (ExperimentSetting.RANDOM_GENERATOR.nextBoolean()) {
							if (ExperimentSetting.RANDOM_GENERATOR.nextBoolean() && numOfTO < 2)
	    	...
	    	if (mutate) {
	    		...
	    		if (random == 0) {
	    			...
	    			if (current.children().size() > 0) {
		    			int to_be_removed = ExperimentSetting.RANDOM_GENERATOR.nextInt(current.children().size());
...
//	    		}
					...
					//0:| 1:U 2:W 3:R 4:M
    				int op = ExperimentSetting.RANDOM_GENERATOR.nextInt(5);
    				if (op == 0)
	...
	public Formula visit(WOperator wOperator) {
	    ...
	    if (numOfAllowedMutations > 0) {
	    	...
	    	if (mutate) {
	    		...
	    		if (random == 0) {
	    			if (ExperimentSetting.RANDOM_GENERATOR.nextInt(2) == 0)
	    				current = left;
	    			else
	    				current = right;
	    		}
...
//		if (numOfInputs > 0)
//			max = numOfInputs;
		int new_variable = ExperimentSetting.RANDOM_GENERATOR.nextInt(max);
...
//		}
		...
		while ((props.contains(new_literal) || props.contains(new_literal.not())) && trying < 5) {
			trying++;
			new_variable = ExperimentSetting.RANDOM_GENERATOR.nextInt(max);
