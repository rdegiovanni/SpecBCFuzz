...
public class FormulaMutator implements Visitor<Formula>{
	private final List<Literal> literalCache;
	  ...
	  public FormulaMutator(List<String> literals, int mutation_rate, int max_num_of_mutations_to_apply, Random random) {
	...
	public Formula visit(BooleanConstant booleanConstant) {
		...
		if (numOfAllowedMutations > 0) { 	
	    	boolean mutate = (this.random.nextInt(mutation_rate) == 0);
	    	if (mutate) {
		...
		if (numOfAllowedMutations > 0) { 	
	    	...
	    	if (mutate) {
	    		...
	    		if (random == 0) {
    				...
    				int op = this.random.nextInt(threshold);
    				if (op == 0)
