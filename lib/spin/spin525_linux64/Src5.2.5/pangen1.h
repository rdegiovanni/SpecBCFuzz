...
static char *Code2d[] = {
	...
	"	{",
#if 1
	/* creates slightly more states and transitions - no clear benefit
	   in a reduction of context switches contributed by the safe moves
	   and, it's not consistent with the assumptions made in the proof
	   of correctness (when returning to state, always make the same
	   selection of a safe transition, so this is disabled by default
	 */
	"#if defined(BCS) && defined(BCS_STICK)",
	"		if  (depth > BASE)	/* there is a prior move */",
	"		{	II = now._last + BASE; /* check last process */",
	"			if (II >= now._nr_pr)  /* process is gone */",
	"			{	goto CheckAllPO;",
	"			}",
	"			this = pptr(II);",
	"			tt = (int) ((P0 *)this)->_p;",
	"			ot = (uchar) ((P0 *)this)->_t;",
	"			if (trans[ot][tt] && trans[ot][tt]->atom & 8) /* safe */",
	"			{	t = trans[ot][tt];",
	"				if (t->qu[0] != 0) /* conditional */",
	"				{	Ccheck++;",
	"					if (!q_cond(II, t))",
	"					{	/* try other safe moves */",
	"						goto CheckAllPO;",
	"					}",
	"					Cholds++;",
	"				}",
	"				/* else: BCS would also select it, but not as a po move */",
	"				goto SelectIt;",
	"				/* if it fails, we continue at Resume,",
	"				   so we don't always try all safe moves",
	"				   in this case -- which is still sound */",
	"		}	}",
	"#endif",
	"CheckAllPO:",
#endif
	...
	"				{",
	"			printf(\"depth %%ld: Claim reached state %%d (line %%d)\\n\",",
...
"#endif",
	...
	"#endif",
#if defined(PC)
	"#if defined(WIN32) || defined(WIN64)",	/* was #if 0 */
#else
	"#if 1",
#endif
	"	tmp = (char *) malloc(n);",
...
"#endif",
	...
	"	{	if (maxdepth > depth)",
	"		{	maxdepth = (iterative == 1)?(depth+1):(depth/2);	/* was ...?(depth-1):... GH 5.2.3 */",
	"		}",
	"		warned = 1;",
...
"#endif",
  ...
  (p.m. unsure if this preserves the guarantee of finding the
   shortest counter-example - so not enabled)
